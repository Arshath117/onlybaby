...
const api_url = "https://onlybaby.onrender.com";
...
export const useAuthStore = create((set) => ({
  ...
  signup: async (email, name, password) => {
    ...
    try {
      const response = await axios.post(
        `https://onlybaby.onrender.com/api/auth/signup`,
        {
          name,
          email,
          password,
  ...
  verifyEmail: async (code) => {
    set({ error: null });
    try {
      const response = await axios.post(`https://onlybaby.onrender.com/api/auth/verifyEmail`, { code });
      const user = response.data.user;
      localStorage.setItem("user", JSON.stringify(user)); // Store user in localStorage
      set({ user, isAuthenticated: true });
    } catch (error) {
  ...
  checkAuth: async () => {
    ...
    try {
      const response = await axios.get(`https://onlybaby.onrender.com/api/auth/check-Auth`);
      const user = response.data.user;
      localStorage.setItem("user", JSON.stringify(user)); // Store user in localStorage
  ...
  login: async (email, password) => {
    set({ error: null });
    try {
      const response = await axios.post(`https://onlybaby.onrender.com/api/auth/login`, {
        email,
        password,
      });
      const user = response.data.user;
